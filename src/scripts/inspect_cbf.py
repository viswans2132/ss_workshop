...
from ss_workshop.srv import SwitchBodyCommand, SwitchBodyCommandResponse
...
class InspectCbf:
    def __init__(self, namespace='', autostart=False, max_speed=1.0, recovery=False):
        ...
        rospy.init_node('inspection_cbf_controller', anonymous=True)
        rospy.loginfo("InspectionCbf node initialized (ROS 1) with namespace: '{}'".format(namespace))
        ...
        self._desired_velocity_world = np.array([0.0, 0.0, 0.0]) # 3D desired position
        self._desired_velocity_body = np.array([0.0, 0.0, 0.0]) # 3D desired position
        ...
        self._desired_yaw_rate = 0.0 # Desired yaw angle
        ...
        self._setvelocity_received = False # Flag to start velocity based control
        self._accept_body_commands = False # Flag to accept body velocity
        ...
        self._setvelocity_subscriber = rospy.Subscriber(
            f"{namespace}/command/velocity", Twist, self.setvelocity_callback, queue_size=1) 
        ...
        self._switch_body_command_server = rospy.Service(f"{namespace}/switch_body_command", SwitchBodyCommand, self.switch_body_command)
    ...
    def _check_control_state_and_stop(self):
        """
        ...
        if not self._odometry_received or not (self._setpoint_received or self._setvelocity_received):
    ...
    def _main_command_callback(self):
        ...
        if self._check_control_state_and_stop():
            return

        if not self._setvelocity_received or not self._takeoff_detected:
            ...
            if self._accept_body_commands:
                ...
                yaw_error = self._desired_yaw_rate/self._k_yaw
    ...
    def odom_callback(self, msg):
        ...
        if self._current_position[2] > self._takeoff_threshold:
            self._takeoff_detected = True

        if not self._odometry_received:
            self._odometry_received = True
            self._desired_position[0] = msg.pose.pose.position.x
            self._desired_position[1] = msg.pose.pose.position.y
    ...
    def setvelocity_callback(self, msg):
        ...
        if not self._setvelocity_received:
            self._setvelocity_received = True
            rospy.loginfo("Velocity Control Initialized.")
    ...
    def switch_body_command(self, req):
        """
        The callback function executed when the service is called.
        ...
        (status of the control output).
        """
        self._accept_body_commands = req.switch
        rospy.loginfo(f"Body Commands: {self._accept_body_commands}")
        return SwitchBodyCommandResponse(status=self._accept_body_commands)
...
def main():
    ...
    try:
        ...
        InspectCbf(namespace=args.namespace, autostart=args.autostart, max_speed=args.max_speed, recovery=args.recovery)
