...
class CbfVelocityController:
    ...
    def _main_command_callback(self):
        ...
        if la.norm(position_error) <= 0.15 and np.abs(yaw_error) <= 0.05: # Threshold of 10 cm
    ...
    def setpoint_callback(self, msg):
        ...
        if (la.norm(new_position - self._desired_position) >= 0.15) or (np.abs(new_yaw - self._desired_yaw) >= 0.02):
    ...
    def pointcloud_callback(self, msg):
        ...
        points = points[rect_mask]
        
        distance_mask = la.norm(points, axis=1) < 3.5
        reduced_points = points[distance_mask]
        ...
        self.points_array = reduced_points
    ...
    def _generate_constraint_matrices(self):
        ...
        X_r = self.points_array[:, 0]
        Y_r = self.points_array[:, 1]
        ...
        translated_points = self.points_array 
